...
abstract class TweetSet {
  ...
  def filter(p: Tweet => Boolean): TweetSet = {
    if (this isEmpty) new Empty else {
      filter0(p, new Empty)
    }
  ...
  def union(that: TweetSet): TweetSet =  {
     if (that.isEmpty ) this else
    	 if (this contains that.head)  this union that.tail 
...
//    if (this.isEmpty) return accu;
//    	else { 
//    		
//    		if (p(elem)) return left filter0(p,accu incl elem ) union right filter0 (p,accu incl elem)
//    		else  left filter0(p,new accu) union right filter0 (p, accu)
//    	}		
//   
//  }
//    if (this.isEmpty ) {println ( this.toString() +"is empty"); return new Empty}
//     if (p(this.head)) {println ( this.head +"is satisfied"); 
//     					this.tail.filter0(p , accu incl this.head); return accu incl this.head } 
//     	else  {this.tail.filter0(p, accu);   return accu  }
//    
//    }
    if (this.isEmpty && !accu.isEmpty) accu 
    else if (this.isEmpty && accu.isEmpty) new Empty else 
    right filter0 (p, left filter0 (p, if (p(elem)) accu.incl(elem) else accu))
    
    
  } 
//     new TreeSet[Int].filter(x => x>10) ;
//     left.filter0(p, accu) foreach(incl) 
//    if (p(elem)) (accu incl elem incl left foreach (p) , right filter0(p,new Empty))
//    else new NonEmpty(accu , left filter0 ( p,new Empty) ,right filter0 (p, new Empty)
//    
//  }
